...
class VocabularyDatabaseStore {
  ...
  Future<void> softDeleteLibrary(String libraryId, DateTime deletedAt) async {
    ...
    await db.transaction((txn) async {
      await txn.update(
        VocabularyDatabaseSchema.librariesTable,
        {'is_deleted': 1, 'deleted_at': timestamp, 'updated_at': timestamp},
        where: 'id = ?',
        whereArgs: [libraryId],
      );
      await txn.update(
  ...
  Future<List<Map<String, Object?>>> listEntries({
  ...
  Future<Map<String, Object?>?> findEntry(String entryId) async {
    final db = await _database;
  ...
  Future<void> saveReviewState(Map<String, Object?> state) async {
    final db = await _database;
    await db.insert(
      VocabularyDatabaseSchema.reviewStatesTable,
      state,
      conflictAlgorithm: ConflictAlgorithm.replace,
    );
  }
